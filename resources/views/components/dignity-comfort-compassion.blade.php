<section class="dignity-section">
    <div class="dignity-container">
        <div class="dignity-image-wrapper">
            <img src="{{ Vite::asset('resources/images/services/pc1.jpeg') }}" alt="A caregiver providing compassionate support to an elderly woman.">
        </div>
        <div class="dignity-content">
            <h2 class="dignity-headline">Providing Dignity, Comfort, and Compassion.</h2>
            <p class="dignity-description">Our mission is to ensure everyone has the support, resources, and companionship they need to live a fulfilling life. We are dedicated to creating a world where aging is embraced with dignity and respect.</p>
            <div class="dignity-tags">
                <span class="dignity-tag">Palliative Care</span>
                <span class="dignity-tag">Geriatric Care</span>
                <span class="dignity-tag">Dementia Care</span>
            </div>
        </div>
    </div>
</section>